...
すべてのデータを削除するには、以下の手順をアプリ内で実施してください：

1. **着想の削除**: 「着想一覧」タブで各着想を左スワイプまたは編集画面から削除
2. **組み合わせカードの削除**: 各カードの詳細画面から削除
3. **アカウント連携の解除**: Apple または Google アカウントを連携している場合、各プロバイダー側の設定画面（Apple ID 設定 / Google アカウント設定）から Idea Brew へのアクセスを取り消してください

上記の操作により、クラウド上のすべてのデータが削除されます。削除されたデータを復元することはできません。
...
To delete all your data, follow these steps within the App:
...
3. **Revoke account linking**: If you linked an Apple or Google account, revoke Idea Brew's access from your provider's settings (Apple ID Settings / Google Account Settings)

These actions will permanently delete all your data from the cloud. Deleted data cannot be recovered.
