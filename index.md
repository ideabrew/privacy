...
また、有害コンテンツ検出のため、入力内容および生成結果を OpenAI のモデレーションAPI（omni-moderation-latest）にも送信します。モデレーション結果はコンテンツ表示可否の判定のみに使用し、当社では保存しません。
...
データを削除するには、以下のいずれかをアプリ内で実施してください：

**個別に削除する場合:**
1. **着想の削除**: 「着想一覧」タブで各着想を左スワイプまたは編集画面から削除
2. **組み合わせカードの削除**: 各カードの詳細画面から削除

**アカウント全削除する場合:**
設定画面の「アカウント削除」から、以下をサーバー側から一括削除できます：
- すべての着想
- すべての組み合わせカード
- 認証情報（匿名ユーザーID、または Apple / Google 連携情報）

**アカウント連携の取消について:**
Apple または Google アカウントを連携していた場合、アカウント全削除を行うと本アプリのサーバー側の認証情報は削除されますが、各プロバイダー側の「サインインしたアプリ」一覧からの取消は別途必要です（Apple ID 設定 / Google アカウント設定より）。

上記の操作により、クラウド上のデータが削除されます。削除されたデータを復元することはできません。
...
In addition, for harmful-content detection, input text and generated output are also sent to OpenAI's Moderation API (omni-moderation-latest). Moderation results are used solely to determine whether content can be displayed, and are not stored on our side.
...
You can delete your data from within the App using either of the following methods:

**Delete individually:**
1. **Delete inspirations**: In the "Inspirations" tab, swipe left on each inspiration or delete from the edit screen
2. **Delete combination cards**: Delete from each card's detail screen

**Delete your entire account:**
From the Settings screen, tap "Delete Account" to remove the following from our servers in one operation:
- All inspirations
- All combination cards
- Authentication information (anonymous user ID, or Apple / Google linked account information)

**About revoking account linking:**
If you linked an Apple or Google account, deleting your account removes the authentication information on our server, but you must separately revoke access from the provider's "Sign in with..." app list (via Apple ID Settings / Google Account Settings).

These actions will permanently delete data from the cloud. Deleted data cannot be recovered.
